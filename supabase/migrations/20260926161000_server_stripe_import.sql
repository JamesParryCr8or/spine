-- Connector members should sync Stripe only through the verified server route,
-- not be able to submit arbitrary revenue entries directly through PostgREST.
drop policy connector_stripe_batch_insert on public.revenue_import_batches;
drop policy connector_stripe_entry_insert on public.revenue_entry_versions;
...
declare batch uuid; begin
  if not (
    (select auth.role()) = 'service_role' and source_name = 'stripe'
  ) and not exists (
