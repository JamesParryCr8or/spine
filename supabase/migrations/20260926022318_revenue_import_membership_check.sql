...
declare batch uuid; begin
  if not exists(select 1 from public.stores s join public.organization_members m on m.organization_id=s.organization_id where s.id=requested_store_id and m.user_id=(select auth.uid()) and m.role in ('owner','admin')) then raise exception 'Owner or admin access required'; end if;
