...
revoke all on function public.save_data_connection(text, text, text, text) from public, anon;
grant execute on function public.save_data_connection(text, text, text, text) to authenticated;
...
begin
  ...
  where membership.user_id = (select auth.uid()) and membership.role in ('owner','admin') and connection.provider = connection_provider
...
revoke all on function public.delete_data_connection(text) from public, anon;
grant execute on function public.delete_data_connection(text) to authenticated;
